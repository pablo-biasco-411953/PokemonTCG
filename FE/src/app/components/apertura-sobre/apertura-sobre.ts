...
export class AperturaSobreComponent implements OnInit, OnDestroy {
  // Contenedor donde Three.js inserta el canvas.
  ...
  private explosionDisparada: boolean = false;
  private animationId!: number;
  private estaCortando: boolean = false;
  ...
  // Inicializa la escena y arranca la animacion del sobre.
  ngOnInit(): void {
    this.initThree();
    this.crearSobrePro();
    this.animate();
  ...
  // Libera recursos graficos al salir del componente.
  ngOnDestroy(): void {
  ...
  // Actualiza la posicion del mouse para luces y gestos.
  ...
  // Prepara camara, renderer y luces principales.
  private initThree() {
  ...
  // Construye el sobre 3D con cuerpo, tapa y linea de corte.
  ...
  // Deforma una geometria para darle mas volumen visual.
  ...
  // Interpreta el arrastre del usuario como un corte progresivo.
  ...
  // Dispara la secuencia final de apertura del sobre.
  ...
  // Decide si el gesto actual es swipe o reveal.
  private procesarSwipeOReveal() {
    ...
    } else if (Math.abs(carta.rotation.y) >= Math.PI - 0.2) {
  ...
  private animate() {
    ...
    if (this.explosionDisparada && this.particulasExplosion) {
