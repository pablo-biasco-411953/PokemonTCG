 ...
 async jugarCarta(carta: any): Promise<void> {
  ...
  validarEnergiaAtaque(ataque: any): boolean {
    ...
    const misEnergias = this.partida.jugador.activo.energiasUnidas.map((e: any) => {
      ...
      return this.normalizarTipoEnergia(texto);
    });

    const costoRequerido = [...ataque.costo].map((t: string) => this.normalizarTipoEnergia(t));
  ...
  getCheckEnergiasAtaque(ataque: any): any[] {
    ...
    const poseidas  = this.partida.jugador.activo.energiasUnidas.map((e: any) => this.normalizarTipoEnergia((e.tipo === 'Energy' || !e.tipo) ? e.nombre : e.tipo));
    const resultado: any[] = [];
    ataque.costo.forEach((tipoRequerido: string) => {
      const tipoNormalizado = this.normalizarTipoEnergia(tipoRequerido);
      const index = poseidas.findIndex((tipo: string) => tipo === tipoNormalizado || tipoNormalizado === 'Colorless');
      if (index !== -1) { resultado.push({ tipo: tipoNormalizado, cumplido: true  }); poseidas.splice(index, 1); }
      else              { resultado.push({ tipo: tipoNormalizado, cumplido: false }); }
    });
    return resultado;
  }
  ...
  getFaltantesAtaque(ataque: any): any[] {
    if (!this.partida?.jugador?.activo?.energiasUnidas) return [];
    const poseidas      = this.partida.jugador.activo.energiasUnidas.map((e: any) => this.normalizarTipoEnergia((e.tipo === 'Energy' || !e.tipo) ? e.nombre : e.tipo));
    const costoRestante = [...ataque.costo].map((tipo: string) => this.normalizarTipoEnergia(tipo));
    const faltantesMap: { [key: string]: number } = {};

    for (let i = costoRestante.length - 1; i >= 0; i--) {
      const tipoReq = costoRestante[i];
      if (tipoReq !== 'Colorless') {
        const index = poseidas.findIndex((tipo: string) => tipo === tipoReq);
  ...
  private normalizarTipoEnergia(tipo: string): string {
    ...
    if (t.includes('lightning') || t.includes('electrica') || t.includes('rayo')) return 'Lightning';
    if (t.includes('psychic')   || t.includes('psiquica')) return 'Psychic';
